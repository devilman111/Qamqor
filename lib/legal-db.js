...
import { ALL_LEGAL_CHUNKS as LEGAL_CHUNKS } from './legal-data.js';
...
export async function setupDatabase() {
  ...
  // Create tables individually (neon tagged-template doesn't support multi-statement)
  await sql`
    CREATE TABLE IF NOT EXISTS legal_chunks (
      id          TEXT PRIMARY KEY,
      source      TEXT NOT NULL,
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS legal_chunks_search_idx ON legal_chunks USING GIN (search_vec)`;
  ...
  `;
  await sql`CREATE INDEX IF NOT EXISTS legal_news_search_idx ON legal_news USING GIN (search_vec)`;
  await sql`CREATE INDEX IF NOT EXISTS legal_news_category_idx ON legal_news (category)`;
  await sql`CREATE INDEX IF NOT EXISTS legal_news_priority_idx ON legal_news (priority)`;
