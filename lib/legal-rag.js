...
export async function getEmbedding(text, taskType = 'RETRIEVAL_QUERY') {
  if (!GOOGLE_API_KEY) {
    console.error('Embedding: GOOGLE_AI_API_KEY не задан');
    return null;
  }
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`Embedding HTTP ${res.status}:`, errBody.slice(0, 300));
      return null;
    }
    const data = await res.json();
...
export async function seedLegalEmbeddings() {
  ...
  if (embedded.length > 0) {
    ...
    // Сбрасываем кэш чтобы изменения сразу применились
...
// Добавить новый chunk в базу (с генерацией эмбеддинга)
export async function addChunkToDb(chunk) {
  if (!chunk.id || !chunk.content) {
    return { ok: false, error: 'id и content обязательны' };
  }
  ...
  let chunks = [];
  if (data) {
    try { chunks = JSON.parse(data); } catch {}
  }

  // Проверить что id уникален
  if (chunks.find(c => c.id === chunk.id)) {
    return { ok: false, error: `Chunk с id "${chunk.id}" уже существует` };
  }
  ...
  const embedding = await getEmbedding(text, 'RETRIEVAL_DOCUMENT');
  if (!embedding) {
    return { ok: false, error: 'Не удалось сгенерировать эмбеддинг' };
  }

  // Добавить и сохранить
  chunks.push({ ...chunk, embedding });
  await redis(['SET', 'legal:knowledge', JSON.stringify(chunks)]);
  _cachedEmbeddings = null;

  return { ok: true, total: chunks.length };
}

// Bulk import — пачкой добавить много chunks
export async function bulkAddChunksToDb(newChunks) {
  if (!Array.isArray(newChunks) || newChunks.length === 0) {
    return { ok: false, error: 'Нужен непустой массив' };
  }
  ...
  for (const chunk of newChunks) {
    if (!chunk.id || !chunk.content) { failed++; continue; }
    if (existingIds.has(chunk.id)) { skipped++; continue; }
  ...
  return { ok: true, added, skipped, failed, total: chunks.length };
}

// Удалить chunk по id
export async function deleteChunkFromDb(id) {
  const data = await redis(['GET', 'legal:knowledge']);
  if (!data) return { ok: false, error: 'База пуста' };

  let chunks;
  try { chunks = JSON.parse(data); }
  catch { return { ok: false, error: 'Ошибка чтения базы' }; }

  const before = chunks.length;
  chunks = chunks.filter(c => c.id !== id);
  if (chunks.length === before) {
    return { ok: false, error: `Chunk с id "${id}" не найден` };
  }
...
// Получить все chunks (без эмбеддингов — экономим трафик)
